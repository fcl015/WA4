...
# Main program
#---------------------------------------------------------------
area2=irrigation_area(2)
print("IRRIGATION AREA 2")

if area2.verbose:
        print("(1) Read configuration files")
wa_config_data.read_config_file(area2.config_file,area2)
wa_config_data.read_operation_mode_file(area2.operation_mode_file,area2)
if area2.verbose:
        print("    OPERATION MODE: "+area2.operation_mode)
...
for i in range(3): 
        wa_xbee_comm.transmit_weather_message(ser,area2,'WEATHER')
        wa_xbee_comm.receive_weather_message(ser,area2,'WEATHER')        
        if area2.comm_status:
                break
        else:
...
for i in range(3): 
        wa_xbee_comm.transmit_sensor_message(ser,area2,'S1')
        wa_xbee_comm.receive_sensor_message(ser,area2,'S1')
        if area2.comm_status:
                break
        else:
                print("    Communication retry....");
...
if area2.comm_status:
        ...
        wa_control.data_fusion(area2)
        wa_control.control_algorithm(area2)

        if area2.verbose:
                print("(6) Transmit control action to actuator")
        for i in range(3): 
                wa_xbee_comm.transmit_actuator_message(ser,area2,'ACTUATOR')
                wa_xbee_comm.receive_actuator_message(ser,area2,'ACTUATOR')
                if area2.comm_status:
                        break
                else:
                        print("    Communication retry....");
...
wa_config_data.update_config_file(area2.config_file,area2)

if area2.verbose:
        print("(8)Update data file")
wa_config_data.update_data_file(area2.data_file,area2)

if area2.verbose:
        print("(9)Update log file")
wa_config_data.update_log_file(area2.log_file,area2)
	
if area2.verbose:
        print("(10)Upload Data to ThingSpeak")
wa_iot_comm.update_thingspeak_area2(area2)
